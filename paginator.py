...
class HelpPaginator(Pages):
    ...
    async def show_page(self, page, *, first=False):
        ...
        for entry in entries:
            addition = entry.short_doc
            if type(entry).__name__=="Group":
                addition+="\n__Subcommands__"
                for command in entry.commands:
                    addition+="\n**"+signature(command)+"**: "+command.short_doc
            self.embed.add_field(name=signature(entry), value=addition or "No help given", inline=False)
